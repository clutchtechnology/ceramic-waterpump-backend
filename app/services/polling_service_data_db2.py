# ============================================================
# 文件说明: polling_service_data_db2.py - DB2 数据轮询服务
# ============================================================
# 功能:
#   1. 读取 DB2 数据块 (6电表 + 1压力 + 6振动)
#   2. 解析和转换传感器数据
#   3. 批量写入 InfluxDB
#   4. 本地缓存降级 (InfluxDB 故障时)
# ============================================================
    ...
    while _is_data_running:
        ...
        try:
            ...
            if not success_db2:
                ...
                logger.error(f"[DB2数据 poll #{poll_count}] 读取失败 (连续 {consecutive_failures} 次): {err_db2}")
            ...
            # 2. 解析数据
            parsed_devices = parse_data_db2(db2_bytes)
