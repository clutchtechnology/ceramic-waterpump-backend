...
def check_disk_space():
    ...
    if free_gb > 10:
        ...
        print(f" 磁盘空间不足: 仅剩 {free_gb:.1f} GB (建议 > 10 GB)")
        return False


def check_firewall():
    """检查 Windows 防火墙（仅提示）"""
    print(" 手动检查: Windows 防火墙需要放行以下端口:")
    print("   - 8081 (FastAPI)")
    print("   - 8086 (InfluxDB)")
    print("   - 102 (PLC S7 通信)")
    return True
...
def check_service_installed():
    ...
    try:
        ...
        if 'RUNNING' in result.stdout:
            ...
            print(" Windows 服务已安装但未运行")
            print("   执行: nssm start CeramicWaterpumpBackend")
            return True
        else:
            print(" 未安装为 Windows 服务")
